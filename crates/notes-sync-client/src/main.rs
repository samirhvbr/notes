...
fn run() -> Result<()> {
    ...
    match command {
        ...
        "recover-client" if args.len() == 3 => {
            let mut remote =
                Remote::connect(&store.endpoint()?, Path::new(&args[2]), ca.as_deref())?;
            let count = store.recover_client(&mut RecoveryRemote(&mut remote))?;
            println!(
                "{}",
                serde_json::json!({"recovered_publications": count, "source_written": false})
            );
        }
        "reconcile-application" if args.len() == 2 => {
            let count = store.reconcile_application_identities()?;
            println!(
                "{}",
                serde_json::json!({"reconciled_receipts": count, "source_written": false})
            );
        }
        "prune-client" if args.len() == 3 => {
            let mut remote =
                Remote::connect(&store.endpoint()?, Path::new(&args[2]), ca.as_deref())?;
            println!(
                "{}",
                serde_json::to_string(&store.prune_client(&mut remote)?)
                    .map_err(|_| Error::Invalid)?
            );
        }
